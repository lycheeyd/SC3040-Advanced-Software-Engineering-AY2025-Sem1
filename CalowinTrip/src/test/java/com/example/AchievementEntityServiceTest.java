...
class AchievementEntityServiceTest {
    ...
    void addTripMetrics_whenUserExists_shouldUpdateExistingAchievement() {
        ...
        AchievementEntity existingAchievementEntity = new AchievementEntity();
        existingAchievementEntity.setTotalCarbonSavedExp(initialCarbonExp);
        existingAchievementEntity.setTotalCalorieBurntExp(initialCalorieExp);

        // Stub the repository to return this existing record
        when(achievementRepository.fetchAchievementForUser(userId)).thenReturn(Optional.of(existingAchievementEntity));
        ...
        AchievementEntity savedAchievementEntity = achievementCaptor.getValue();

        // Check that the saved object has the correctly updated totals
        assertThat(savedAchievementEntity.getTotalCarbonSavedExp()).isEqualTo(initialCarbonExp + carbonToAdd); // 100 + 50 = 150
        assertThat(savedAchievementEntity.getTotalCalorieBurntExp()).isEqualTo(initialCalorieExp + caloriesToAdd); // 200 + 75 = 275
    ...
    void addTripMetrics_whenUserIsNew_shouldCreateNewAchievement() {
        ...
        assertThat(savedAchievementEntity.getTotalCarbonSavedExp()).isEqualTo(carbonToAdd);
        assertThat(savedAchievementEntity.getTotalCalorieBurntExp()).isEqualTo(caloriesToAdd);
    }
    ...
    void getAchievementProgress_whenUserExists_shouldReturnAchievementResponse() {
        ...
        AchievementEntity existingAchievementEntity = new AchievementEntity();
        existingAchievementEntity.setTotalCarbonSavedExp(1500); // Should be EcoBronze
        existingAchievementEntity.setTotalCalorieBurntExp(5500); // Should be CalorieSilver
