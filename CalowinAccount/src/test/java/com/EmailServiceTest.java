...
/**
 * FINAL FIX: This configuration resolves the "Broken pipe" and "Javax/Jakarta" API conflicts
 * by creating a test-only version of the GmailSmtp service with simplified properties.
 */
...
class EmailServiceTest {
    ...
    // Static block to suppress conflicting mail providers (IMAP/POP3) during Session init
    static {
...
// ...
        ...
        // Note: Using jakarta.mail.internet.MimeMessage.RecipientType.TO from your corrected imports
        assertThat(receivedMessage.getSubject()).isEqualTo(subject);
        assertThat(((InternetAddress) receivedMessage.getRecipients(jakarta.mail.Message.RecipientType.TO)[0]).getAddress()).isEqualTo(recipient);
     ...
     * This implementation uses simplified properties (no TLS/SSL) and correct Jakarta APIs.
     */
    @TestConfiguration
