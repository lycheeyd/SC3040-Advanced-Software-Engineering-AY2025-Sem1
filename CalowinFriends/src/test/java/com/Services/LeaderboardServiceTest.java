...
class LeaderboardServiceTest {
        ...
        private Achievement createAchievement(String userId, String userName, int carbon, int calories) {
                ...
                // Mock names are set here for simplicity, but in the service, they are fetched
                // separately
                return ach;
        }
        ...
        void getFriendsIds_ShouldReturnAcceptedFriends() {
                // ARRANGE
                String userId = "user1";
                when(friendRelationshipRepository.findByIdUniqueIdOrIdFriendUniqueIdAndStatus(userId, userId,
                                "ACCEPTED"))
        ...
        void getCarbonLeaderboard_ShouldReturnSortedByCarbon() {
                ...
                when(jdbcTemplate.queryForObject(eq(sql), eq(new Object[] { "user1" }), eq(String.class)))
                                .thenReturn("User One");
                when(jdbcTemplate.queryForObject(eq(sql), eq(new Object[] { "friend1" }), eq(String.class)))
                                .thenReturn("Friend One");
                when(jdbcTemplate.queryForObject(eq(sql), eq(new Object[] { "friend2" }), eq(String.class)))
                                .thenReturn("Friend Two");
                // --- END OF FIX ---
        ...
        void getCaloriesLeaderboard_ShouldReturnSortedByCalories() {
                ...
                // Verify it is sorted by totalCalorieBurnt in descending order (3000, 2500,
                // 2200)
        ...
        void getFriendsIds_WhenUserHasNoFriends_ShouldReturnOnlySelf() {
                ...
                // ASSERT
                // The list should contain only the user themselves, as per the logic
