...
public class FriendRelationshipService {
    ...
    public List<FriendRelationshipDTO> getPendingRequests(String userId) {
        // Fetch only requests where the userId is in the Friend_Unique_ID column and
        // status is "PENDING"
    ...
    public List<FriendRelationshipDTO> getFriendList(String userId) {
        ...
        return repository.findByIdUniqueIdOrIdFriendUniqueIdAndStatus(userId, userId, "ACCEPTED").stream()
                .filter(relationship -> "ACCEPTED".equals(relationship.getStatus())) // Ensure only accepted
                                                                                     // relationships
                .map(relationship -> {
                    ...
                    // Convert to DTO using friend ID and name (prevent adding the userId itself to
                    // the friend list)
                    return new FriendRelationshipDTO(
                            userId,
    ...
    public FriendStatus getRelationshipStatus(String userId1, String userId2) {
        Optional<FriendRelationship> directRelationship = repository
                .findById(new FriendRelationshipId(userId1, userId2));
        Optional<FriendRelationship> reverseRelationship = repository
                .findById(new FriendRelationshipId(userId2, userId1));
