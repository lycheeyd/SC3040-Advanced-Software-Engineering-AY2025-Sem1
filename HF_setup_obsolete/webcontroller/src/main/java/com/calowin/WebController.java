...
public class WebController {
    ...
    private static final String INTERNAL_SERVER_URL = "http://localhost:8080";
...
<head>
    ...
    <title>Calowin Backend API Demo</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
""";
    ...
    /**
     * Catch-all method to forward all other requests to internal server on port 8080
     * This method handles all HTTP methods and paths except the root path "/"
     */
    ...
    public ResponseEntity<String> forwardRequest(
        ...
        try {
            ...
            // Construct the full URL for the internal server
            String targetUrl = INTERNAL_SERVER_URL + requestPath;
            ...
            request.getHeaderNames().asIterator().forEachRemaining(headerName -> {
                if (!headerName.equalsIgnoreCase("host")) { // Skip host header
            ...
            return ResponseEntity
                .status(500)
                .body("Error forwarding request to internal server: " + e.getMessage());
        }
    }
}
